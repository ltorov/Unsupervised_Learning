...
class SubtractiveClustering:
    def __init__(self, alpha_radius: float = 0.5, beta_radius: float = None, max_iterations: int = 100, centers: np.array = None):
        """
    ...
    def assign_to_centers(self, data: np.array) -> np.array:
        ...
        for i, xi in enumerate(data):
            ...
            for cj in self.centers:
                Di = np.exp(-(np.linalg.norm(xi - data[cj]) ** 2) / ((self.alpha_radius/2) ** 2))
                D.append(Di)
